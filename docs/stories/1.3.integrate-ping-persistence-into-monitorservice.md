...
- [ ] Task 1: Create PingRepository Service (AC: 1, 3, 4)
  - [ ] 1.1 Create file `netmonitor/src/app/services/ping.repository.ts`
  - [ ] 1.2 Inject `DatabaseService` using `inject()` pattern
  - [ ] 1.3 Implement `savePing(result: PingResult, target: string): Promise<void>` method
    - [ ] 1.3.1 Convert `PingResult` to database row format:
    ...
    - [ ] 1.3.2 Use parameterized INSERT: `INSERT INTO pings (timestamp, latency_ms, success, target) VALUES (?, ?, ?, ?)`
  - [ ] 1.4 Implement error handling within `savePing()`:
    - [ ] 1.4.1 Check `databaseService.isInitialized` at start - return early if false
    - [ ] 1.4.2 Wrap database call in try/catch
    - [ ] 1.4.3 Log errors with `console.error('Failed to persist ping:', err)`
    - [ ] 1.4.4 Never throw - always resolve (errors logged, not propagated)
  - [ ] 1.5 Add JSDoc documentation for public methods

- [ ] Task 2: Integrate PingRepository into MonitorService (AC: 1, 2)
  - [ ] 2.1 Inject `PingRepository` into `MonitorService` using `inject()` pattern
  - [ ] 2.2 Modify the subscribe block in `startMonitoring()` to call `savePing()` after updating results
  - [ ] 2.3 Implement non-blocking persistence (fire-and-forget pattern):
...
- [ ] Task 3: Write Unit Tests for PingRepository (AC: 1, 3, 4)
  - [ ] 3.1 Create `netmonitor/src/app/services/ping.repository.spec.ts`
  - [ ] 3.2 Test: `savePing()` calls `DatabaseService.execute()` with correct SQL and params
  - [ ] 3.3 Test: `savePing()` converts timestamp to Unix epoch correctly
  - [ ] 3.4 Test: `savePing()` converts success status to 1/0 INTEGER
  - [ ] 3.5 Test: `savePing()` handles null latencyMs for error results
  - [ ] 3.6 Test: `savePing()` catches and logs database errors (doesn't throw)
  - [ ] 3.7 Test: `savePing()` returns early when database not initialized (no execute call)

- [ ] Task 4: Write Integration Tests for MonitorService Persistence (AC: 2, 3)
  - [ ] 4.1 Update `netmonitor/src/app/services/monitor.service.spec.ts`
  - [ ] 4.2 Test: Monitoring emits results via `results$` regardless of database state
  - [ ] 4.3 Test: `savePing()` is called after each ping result
  - [ ] 4.4 Test: Database errors don't stop monitoring (verify `results$` continues emitting)
  - [ ] 4.5 Test: Ping timing is not affected by database write (non-blocking verification)

- [ ] Task 5: Verify Regression - Existing Functionality (IV: 1, 2, 3)
  - [ ] 5.1 Run existing test suite: `npm test` - all tests pass
  - [ ] 5.2 Verify in browser: app builds and runs without database (WebDatabaseService no-op) (IV1)
  - [ ] 5.3 Manual verification: chart updates at expected rate (2 second default interval) (IV1)
  - [ ] 5.4 Tauri verification: Build and run desktop app, confirm pings are persisted (AC5)
    - [ ] 5.4.1 Run app for 30+ seconds
    - [ ] 5.4.2 Stop app
    - [ ] 5.4.3 Verify data persists by checking database file or restarting app
  - [ ] 5.5 Lint check: `npm run lint` passes
